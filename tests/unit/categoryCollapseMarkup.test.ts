...
describe('category hierarchy visibility markup', () => {
  it('shows every root group with direct bookmarks and per-group child tabs', () => {
    ...
    expect(section).toContain('showEditButton={Boolean(onEditBookmark) && activeSortMode}')
    expect(section).toContain('export let showSortSessionLabel = true')
    ...
    expect(card).toContain('{#if canEdit && showEditButton && onEdit}')
    expect(card).toContain('.bookmark-card-shell.sort-mode .bookmark-edit-button')
    ...
    expect(home).toContain('{#if homeSortMode}')
    expect(home).toContain('class="sort-category-list"')
    expect(home).toContain('bookmarks={displayCategoryBookmarks.get(category.id) ?? []}')
    expect(home).toContain('bookmarks={displayCategoryBookmarks.get(child.id) ?? []}')
